...
}var url ="https://newsapi.org/v2/top-headlines?sources=business-insider&apiKey=0829585b678c492ea688ce9899e48313";
	...
	xmlhttp.onreadystatechange = function(){
		...
		if(xmlhttp.readyState == 4 && xmlhttp.status == 200){
			...
			if(xmlhttp.responseText==="Nothing in the List"){
				var resp="<div class='col-sm-12'>" +"<h1>Nothing in the List</h1>"+"</b></div>";
			...
			for(i=0;i<finalObj.news.length;i++){
				res=res+"<div class='col-lg-4 col-sm-6'>" +
				
				"<div><b>"+finalObj.news[i].title+"</b></div>"+
				"<div class='row'>"+
				"<div class='col-sm-6'>"+
				"<a href="+finalObj.news[i].url+">"+
				"<img src="+finalObj.news[i].urlToImage+" alt='No Image'></div>"+
				"<div class='col-sm-6'>"+finalObj.news[i].description+"</div>"+
				"</a>" +
				"</div>"+
				"<input class='add' type='submit' value='Remove' onclick=\"addToFavourites(this)\" onclick=\"favourites()\" " +
				"flag=\"" +flag+"\""+
				"title=\""+finalObj.news[i].title+"\" " +
				"url=\"" +finalObj.news[i].url+"\" " +
				"urlToImage=\"" +finalObj.news[i].urlToImage+"\"" +
				"description=\"" +finalObj.news[i].description+"\"></div>";
...
function addToFavourites(input){
	...
	var url1 ="http://localhost:8081/AllRoundNews/Handler?title="+title+
	"&url="+url+"&urlToImage="+urlToImage+"&description="+description+"&flag="+flag;
